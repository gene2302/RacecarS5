...
class slash_controller(object):
    ...
    def __init__(self):
        ...
        self.K_autopilot = np.array ([[0,0,8.2335],[0.3162, 0.8558, 0]])
        self.K_parking = np.array ([[0,0,1],[0.0938,0.3,0]])
        ...
        self.steering_ref    = 0
        self.position_ref = 4   #position autopilot stationnement
    ...
    def timed_controller(self, timer):
        ...
        if (self.high_level_mode == 0 ):
            ...
            if  ( self.high_level_mode == 1 ):
                ...
                r = np.array([0,0,self.propulsion_ref])
                
                N = np.array([[10.1895, 0],[0 , 0.3162]])
                u = self.autopilot1( y , r )

                #u = (np.dot(N,np.array([y[2],y[0]])) - np.dot(self.K_autopilot,y))
                ...
                y = np.array([ -self.laser_y , self.laser_theta,self.position ])
                r = np.array([0,0,self.position_ref])
